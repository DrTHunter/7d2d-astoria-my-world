#!/usr/bin/env python3
"""
Turn a stock Astoria 8K world into DrTHunter's version.

    python tools/install.py

This does the one thing Vortex cannot: patch the map itself. The starter-base
mod is a normal Vortex mod - see vortex/Astoria-StarterBases.zip.
...
<name>.stock-backup first, so you can always go back.
    ...
    --no-vortex     also drop the starter-base mod straight into Mods\\
...
MODZIP = os.path.join(ROOT, "vortex", "Astoria-StarterBases.zip")
...
def undo(world, man):
    ...
    print("The starter-base mod is separate - turn it off in Vortex.")
...
def main():
    ...
    ap.add_argument("--no-vortex", action="store_true",
                    help="also copy the starter-base mod into Mods\\ yourself")
    ...
    if a.no_vortex:
        ...
        print(f"\ninstalling the starter-base mod -> {os.path.join(dst,'Astoria-StarterBases')}")
        old = os.path.join(dst, "Astoria-StarterBases")
